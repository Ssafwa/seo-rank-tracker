import { useState, useEffect } from "react"; // Added useEffect to track state prop updates
import { Link, useNavigate, useSearchParams } from "react-router-dom"; // Removed unused component Navigate import
...
export default function Login({ state }: { state: string }) {
    ...
    // Sync local state if the user navigates between /login and /register paths externally
    useEffect(() => {
        setIsLoginState(state === "login");
    }, [state]);

    const handleSubmit = async (e: React.FormEvent) => { // Fixed type from React.SubmitEvent to React.FormEvent
        e.preventDefault();
        setLoading(true);
        ...
        if (isLoginState) {
            ...
            result = await register(name, email, password); // Fixed: Called register instead of login
        }

        if (result.success) {
            const redirect = searchParams.get("redirect") || "/dashboard";
            navigate(redirect); // Fixed: Used lowercase hook variable navigate() instead of Navigate
        } else {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="text-center py-5">
                            <h1 className="text-2xl text-foreground">
                                {isLoginState ? "Welcome back" : "Get Started"}
                            </h1>
