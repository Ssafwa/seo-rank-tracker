...
import { 
    ...
    AlertCircle, Eye, EyeOff, Filter, ArrowUpDown 
} from "lucide-react";
import { useApp } from "../context/AppContext";

// Define your interface once
interface KeywordItem {
    _id: string;
    // ... rest of your interface properties
}
interface KeywordItem {
    _id: string;
    keyword: string;
...
const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
        ...
        console.log("Full API Response:", res.data); // DEBUG: Check what the backend actually sends
        
        if (res.data.success) {
            // SAFE UPDATE: Use res.data.tracking OR res.data if tracking is missing
            const newItem = res.data.tracking || res.data; 
            ...
            setAddError(res.data.message || "Failed to add tracking.");
        }
    } catch (err: any) {
    ...
    const handleRefresh = async (id: string) => {
        setRefreshing(id);
        try {
            setKeywords(prev => prev.map(k => k._id === id ? { ...k, lastCheck: "checking" } : k));
            const res = await api.post(`/api/rank/${id}/refresh`);
            if (res.data.success) {
                const pollInterval = setInterval(async () => {
                    try {
                        const check = await api.get(`/api/rank/${id}`);
                        if(check.data.tracking.lastCheck !== "checking") {
...
const handleDelete = async (id: string) => {
        ...
        try {
            ...
            console.error("Delete tracking item error:", err);
        }
        setDeleting(null);
    };
    ...
    // This now sits AFTER all functions, but BEFORE the return statement
    const safeKeywords = Array.isArray(keywords) ? keywords : [];
    let processedData = [...safeKeywords];
    ...
    if (statusFilter !== "all") {
        processedData = processedData.filter((k) => 
            statusFilter === "active" ? k.active === true : k.active === false
        );
    }
...
// Sort Data
    processedData.sort((a: any, b: any) => {
        if (sortBy === "newest") {
            ...
            // FIXED: Ensure you compare 'a' and 'b' correctly here
    ...
    // Single consolidated helper functions
    ...
    // ... (Remaining code including useEffect and return)
                    ...
                    {kw.lastCheck === "checking" ? (
                        ...
                        <div className={`w-16 h-16 rounded-xl flex items-center justify-center text-lg font-bold ${posBadge.class}`}>
                            {kw.currentPosition ? `#${kw.currentPosition}` : "—"}
                                            ...
                                            {kw.lastCheck === "completed" && kw.currentPosition && (
                                        ...
                                        <div className="flex items-center gap-1.5 shrink-0">
                                            ...
                                            <button onClick={() => handleRefresh(kw._id)} disabled={refreshing === kw._id || kw.lastCheck === "checking"} className="p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-all disabled:opacity-30" title="Refresh Ranking">
